...
public static class GeneralRole
{
    ...
    public static AppRole GeneralStaffRole { get; } = new(GeneralStaff, GeneralCategory, AppRole.RoleType.Staff,
        "General Staff", "Can access search results and read-only views of most data.");

    public static AppRole SiteMaintenanceRole { get; } = new(SiteMaintenance, GeneralCategory,
        AppRole.RoleType.SiteMaintenance, "Site Maintenance",
        "Can update values in common lookup tables (drop-down lists).");

    public static AppRole UserAdminRole { get; } = new(AppUserAdmin, GeneralCategory, AppRole.RoleType.UserAdmin,
