...
public class IaipSourceTestService(IDbConnectionFactory dbf) : ISourceTestService
{
    public async Task<BaseSourceTestReport?> FindAsync(int referenceNumber) =>
        !await SourceTestExistsAsync(referenceNumber).ConfigureAwait(false)
            ? null
            : await GetSourceTestReportFromDb(referenceNumber).ConfigureAwait(false);
    ...
    private async Task<BaseSourceTestReport?> GetSourceTestReportFromDb(int referenceNumber) =>
        await GetDocumentTypeAsync(referenceNumber).ConfigureAwait(false) switch
        {
            ...
            DocumentType.OneStackTwoRuns or DocumentType.OneStackThreeRuns or DocumentType.OneStackFourRuns =>
                await GetOneStackAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.TwoStackStandard or DocumentType.TwoStackDre =>
                await GetTwoStackAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.LoadingRack => await GetLoadingRackAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.PondTreatment => await GetPondTreatmentAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.GasConcentration => await GetGasConcentrationAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.Flare => await GetFlareAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.Rata => await GetRataAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.MemorandumStandard or DocumentType.MemorandumToFile or DocumentType.PTE =>
                await GetMemorandumAsync(referenceNumber).ConfigureAwait(false),
            DocumentType.Method9Multi or DocumentType.Method22 or DocumentType.Method9Single =>
    ...
    public async Task<SourceTestSummary?> FindSummaryAsync(int referenceNumber) =>
        !await SourceTestExistsAsync(referenceNumber).ConfigureAwait(false)
            ? null
            : await GetSourceTestSummaryFromDb(referenceNumber).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyCollection<SourceTestSummary>> GetSourceTestsForFacilityAsync(FacilityId facilityId)
    {
        ...
        var multi = await db.QueryMultipleAsync("air.GetFacilitySourceTests",
            param: new { FacilityId = facilityId.Id }, commandType: CommandType.StoredProcedure).ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);
        ...
        string? assignmentUser, Guid? assignmentOffice, int skip, int take)
    {
        ...
        var count = await multi.ReadSingleAsync<int>().ConfigureAwait(false);
    ...
    private async Task<T> GetBaseSourceTestReportAsync<T>(int referenceNumber) where T : BaseSourceTestReport
    {
        ...
        var multi = await db.QueryMultipleAsync("air.GetBaseSourceTestReport",
            param: new { referenceNumber }, commandType: CommandType.StoredProcedure).ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);
        ...
        report.WitnessedByStaff.AddRange(await multi.ReadAsync<PersonName>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportOneStack> GetOneStackAsync(int referenceNumber)
    {
        ...
        var multi = await getMultiTask.ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);

        _ = multi.Read<SourceTestReportOneStack, ValueWithUnits, ValueWithUnits, ValueWithUnits, ValueWithUnits,
        ...
        report.AllowableEmissionRates.AddRange(await multi.ReadAsync<ValueWithUnits>().ConfigureAwait(false));
        report.TestRuns.AddRange(await multi.ReadAsync<StackTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportTwoStack> GetTwoStackAsync(int referenceNumber)
    {
        ...
        report.AllowableEmissionRates.AddRange(await multi.ReadAsync<ValueWithUnits>().ConfigureAwait(false));
        report.TestRuns.AddRange(await multi.ReadAsync<TwoStackTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportPondTreatment> GetPondTreatmentAsync(int referenceNumber)
    {
        ...
        _ = multi.Read<SourceTestReportPondTreatment, ValueWithUnits, ValueWithUnits, ValueWithUnits, ValueWithUnits,
        ...
        report.TestRuns.AddRange(await multi.ReadAsync<PondTreatmentTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportGasConcentration> GetGasConcentrationAsync(int referenceNumber)
    {
        ...
        _ = multi.Read<SourceTestReportGasConcentration, ValueWithUnits, ValueWithUnits, ValueWithUnits, ValueWithUnits,
        ...
        report.AllowableEmissionRates.AddRange(await multi.ReadAsync<ValueWithUnits>().ConfigureAwait(false));
        report.TestRuns.AddRange(await multi.ReadAsync<GasConcentrationTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportFlare> GetFlareAsync(int referenceNumber)
    {
        ...
        var report = await GetBaseSourceTestReportAsync<SourceTestReportFlare>(referenceNumber).ConfigureAwait(false);

        var multi = await getMultiTask.ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);

        _ = multi.Read<SourceTestReportFlare, ValueWithUnits, ValueWithUnits, ValueWithUnits, ValueWithUnits,
        ...
        report.AllowableEmissionRates.AddRange(await multi.ReadAsync<ValueWithUnits>().ConfigureAwait(false));
        report.TestRuns.AddRange(await multi.ReadAsync<FlareTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestReportRata> GetRataAsync(int referenceNumber)
    {
        ...
        var report = await GetBaseSourceTestReportAsync<SourceTestReportRata>(referenceNumber).ConfigureAwait(false);

        var multi = await getMultiTask.ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);

        var r = await multi.ReadSingleAsync<SourceTestReportRata>().ConfigureAwait(false);
        ...
        report.TestRuns.AddRange(await multi.ReadAsync<RataTestRun>().ConfigureAwait(false));
    ...
    private async Task<SourceTestMemorandum> GetMemorandumAsync(int referenceNumber)
    {
        ...
        var report = await GetBaseSourceTestReportAsync<SourceTestMemorandum>(referenceNumber).ConfigureAwait(false);
    ...
    private async Task<SourceTestReportOpacity> GetOpacityAsync(int referenceNumber)
    {
        ...
        var report = await GetBaseSourceTestReportAsync<SourceTestReportOpacity>(referenceNumber).ConfigureAwait(false);

        var multi = await getMultiTask.ConfigureAwait(false);
        await using var multiAsyncDisposable = multi.ConfigureAwait(false);

        var r = await multi.ReadSingleAsync<SourceTestReportOpacity>().ConfigureAwait(false);
        ...
        report.TestRuns.AddRange(await multi.ReadAsync<OpacityTestRun>().ConfigureAwait(false));
