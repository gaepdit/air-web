...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (event) {
        ...
        if (today.getMonth() === 9 && today.getDate() <= 14) { // If today is within the first two weeks of October.
            const selectedYear = document.querySelector('select[name="Item.Year"]').value;
            const yearDescription = Number(selectedYear) === today.getFullYear() ? "recently ended September 30" : "just began October 1";
