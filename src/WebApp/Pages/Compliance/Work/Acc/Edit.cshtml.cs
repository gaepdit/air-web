...
public class EditModel(
    ...
    IValidator<AccUpdateDto> validator) : PageModel
{
    [FromRoute]
    public int Id { get; set; }

    [BindProperty]
    public AccUpdateDto Item { get; set; } = default!;
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var item = (AccUpdateDto?)await entryService.FindForUpdateAsync(Id);
    ...
    {
        var original = (AccUpdateDto?)await entryService.FindForUpdateAsync(Id, token);
        ...
        const WorkEntryType entryType = WorkEntryType.AnnualComplianceCertification;
    ...
    private Task<bool> UserCanEditAsync(AccUpdateDto item) =>
        authorization.Succeeded(User, item, new AccUpdateRequirement());
}
