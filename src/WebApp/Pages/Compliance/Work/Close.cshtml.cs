...
public class CloseModel(IWorkEntryService entryService, IAuthorizationService authorization) : PageModel
{
    [FromRoute]
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        if (!await UserCanCloseAsync(item)) return Forbid();
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken token)
    {
        ...
        if (item is null || !await UserCanCloseAsync(item))
            return BadRequest();
    ...
    private Task<bool> UserCanCloseAsync(WorkEntrySummaryDto item) =>
        authorization.Succeeded(User, item, ComplianceWorkOperation.Close);
}
